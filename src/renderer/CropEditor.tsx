...
export function CropEditor({ asset, busy, save, remove, close }: { asset: MediaAsset; busy: boolean; save: (edits: Edits) => void; remove: () => void; close: () => void }) {
  ...
  return <Modal title="调整画面" onClose={close} busy={busy}>
    <div className="crop-stage"><div ref={container} className="crop-canvas" style={{ width: Math.min(600, 330 * dimensions.width / dimensions.height), aspectRatio: `${dimensions.width}/${dimensions.height}` }}>
      <canvas ref={canvas}/><div className="crop-selection" style={{ left: `${crop.x * 100}%`, top: `${crop.y * 100}%`, width: `${crop.width * 100}%`, height: `${crop.height * 100}%` }}
        ...
        onPointerUp={() => drag.current = null} onLostPointerCapture={() => drag.current = null}></div>
    </div></div>
    <div className="crop-settings"><div className="controls"><button disabled={busy} onClick={() => setEdits({ rotation: ((edits.rotation + 90) % 360) as Edits['rotation'] })}>旋转 90°</button><button disabled={busy} onClick={() => setEdits({ rotation: 0 })}>恢复原图</button></div>
