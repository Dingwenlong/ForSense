...
0.1.6 移除左侧每张图片的移动、编辑和移除按钮。单击预览图片的编辑弹窗提供裁切、旋转与移除；预览区仍支持拖放排序。左侧素材获得焦点后可按 Alt + ↑ / ↓ 排序。

0.1.7 移除工作台左侧编辑栏。微信预览内可直接输入文案，九宫格提供“＋添加图片”占位；抖音预览点击正文编辑，画面提供“＋添加图片”占位。标题、平台、视频取材、复制文案、草稿操作与导出位于预览页顶部。
